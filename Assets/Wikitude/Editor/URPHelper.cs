...
namespace WikitudeEditor
{
    ...
    public class URPHelperItem : EditorWindow
    {
        ...
        private static string _urlToDocumentation = "https://www.wikitude.com/external/doc/expertedition/docs/SetupGuide.html#universal-render-pipeline-support";
#if URP_ENABLED
        ...
        private void CalcListOfScenes() {       
            var directoryInfo = new DirectoryInfo(Application.dataPath + "/Wikitude/Samples/Scenes");
            ...
            for (int i = 0; i < _sceneFiles.Count; i++) {
                try {
                    ...
                    if (mainCameraData.renderType == CameraRenderType.Overlay && cameraFrameRendererCameraData.cameraStack.Contains(Camera.main)) {
                        Debug.LogWarning("URP relevant changes already set in scene: " + _sceneFiles[i].Name);
